...
export function PresentationGrid({ presentations }: { presentations: BackendPresentation[] }) {
  ...
  useEffect(() => {
    ...
    const newFiltered = presentations
      .filter(presentation => {
    ...
    <Box display="flex" flexDirection="row" ml={3} gap={2}>
      ...
      <FormControl sx={{ minWidth: "220px" }}>
        ...
        <Select
          ...
          value={keywordFilter}
          label="Keywords"
          onChange={(e) => setKeywordFilter(typeof e.target.value === "string" ? e.target.value.split(",") : e.target.value)}
        ...
        >
          {allKeywords.map((keyword) => (
            <MenuItem key={keyword} value={keyword} sx={{ p: 0, pr: 2 }}>
              ...
              <ListItemText primary={keyword} slotProps={{ primary: { pb: 0 } }} />
            </MenuItem>
          ))}
        </Select>
      </FormControl>
      ...
      {(eventFilter != "" || keywordFilter.length !== 0) &&
        <Box height="100%">
          <Button variant="outlined" onClick={() => { setEventFilter(""); setKeywordFilter([]); }} sx={{ display: "block" }}>
            Reset Filters
          </Button>
        </Box>
      }
    ...
    <Container maxWidth={"xl"} sx={{ mt: 3 }}>
      <Grid container spacing={1}>
        {filteredPresentations.map((presentation) => (
          <Grid key={presentation.slug.join("-")} size={{ xs: 12, md: 6, lg: 4 }}>
            <PresentationCard
